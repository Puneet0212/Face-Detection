...
export const saveFaceMatched = async (known: string, unknown: string) => {
  await faceDetectionNet.loadFromDisk("./weights");
  await faceapi.nets.faceLandmark68Net.loadFromDisk("./weights");
  await faceapi.nets.faceRecognitionNet.loadFromDisk("./weights");

  const referenceImage = ((await canvas.loadImage(
    known || REFERENCE_IMAGE
  ...
  const newbestMatch = faceMatcher.findBestMatch(resultsQuery[0].descriptor);
  console.log("***** Works For ONLY ONE FACE *****");
  console.log(newbestMatch.label, " : ", newbestMatch.distance);

  return newbestMatch.distance;
};
